...
class DeathDetectorRB(object):
	...
	def get_death_information(self, text):
		### normalize text
		...
		for sentence in sentence_split:
			...
			for word in self.death_dictionary:
				if (word in (' '.join(tokens).lower()).split(' ')):
					death_id = (' '.join(tokens).lower()).split(' ').index(word)
			...
			# 
			sen_cnt = []
			for i in range(death_id - 1, -1, -1):
				if ((tokens[i] == ',') or (tokens[i].lower() in self.injury_dictionary)):
					break
				if (tokens[i].lower() in self.human_dictionary):
					...
					sen_cnt.append(1)
			if (len(sen_cnt) != 0):
				number_died.add(sum(sen_cnt))
		### return
		number_died = list(number_died)
		if not exist:
			return 0
		else:
			if len(number_died) == 1:
				return number_died[0]
			else:
				return None
